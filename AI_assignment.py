...
def initial(file,tmpState,hCost,ckk):
    ...
    global call
    call = 0
    tmpNode = node(state=tmpState, parent=-1, action=getAction(srtX, srtY), g_cost=0, depth=0, h_cost=hCost, x=srtX, y=srtY,
                   move="intit",ck=ckk)
    Node += [tmpNode]
    isVisited.clear()
...
def prntResult(file,nd,elapsedTime,num):
    ...
    if nd == None:
        ...
        file.write("Total call: " + str(call) + "\nTotal node Create: " + str(indx) + "\nElapsed Time : " + str(
            elapsedTime) + "\n")
        pfile.write(str(num)+" "+" "+str(indx)+" "+"%.2f" % round(elapsedTime,2)+" #\n")
    else:
        ...
        pfile.write(str(num)+" "+str(nd.depth)+" "+str(indx)+" "+"%.2f" % round(elapsedTime,2)+"\n")

def call_BFS(tmpState):
    file = open("output_BFS.txt", "w")

    tmpNode=initial(file,tmpState,0,0)
    ...
    prntResult(file,nd,elapsedTime,1)
...
def call_UCS(tmpState):
    ...
    prntResult(file,nd,elapsedTime,2)
...
def call_DLS(tmpState,limit):
    ...
    if nd == None:
        ...
        file.write("Total call: " + str(call) + "\nTotal node Create: " + str(indx) + "\nElapsed Time : " + str(
            elapsedTime) + "\n")
        pfile.write(str(3) + " "+ " " + str(indx) + " " + "%.2f" % round(elapsedTime, 2) + " #\n")
        ...
        for nn in answer:
            file.write("move :" + nn.move + "\n")
            filePrint(file, nn.state)
        pfile.write(str(3) + " " + str(nd.depth) + " " + str(indx) + " " + "%.2f" % round(elapsedTime, 2) + "\n")
    file.close()
    ...
    for depthh in range(0,100):
        ...
        if nd == None:
            ...
            file.write("Total call: " + str(call) + "\nTotal node Create: " + str(nodeCount) + "\nElapsed Time : " + str(
                elapsedTime) + "\n")
            pfile.write(str(4) + " "+ " " + str(indx) + " " + "%.2f" % round(elapsedTime, 2) + " #\n")
            break
        elif nd != cutOffNode:
            print("End of DLS...........")
            elapsedTime = time.time() - startTime
            ...
            for nn in answer:
                file.write("move :" + nn.move + "\n")
                filePrint(file, nn.state)
            pfile.write(str(4) + " " + str(nd.depth) + " " + str(indx) + " " + "%.2f" % round(elapsedTime, 2) + "\n")
            break
    file.close()
...
def call_GBFS(tmpState,chk):
    ...
    tmpNode=initial(file,tmpState,cost,1)
    ...
    prntResult(file,nd,elapsedTime,5)
...
def call_A_star(tmpState,chk):
    ...
    cost = heuristic(tmpState, chk)
    tmpNode=initial(file,tmpState,cost,2)
    ...
    prntResult(file,nd,elapsedTime,6)

    file.close()

while True:

    OnOROFF = int(input("Enter Choice\n1.Online Test\n2.Offline\n"))
    if OnOROFF == 2:
        offLine()
    elif OnOROFF == 1:
        inputFrom = int(input("Enter Choice\n1.Input from Console\n2.Input from File\n"))
        if inputFrom==1:
